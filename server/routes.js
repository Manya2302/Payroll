...
export function createRoutes(app) {
  ...
  app.put("/api/employees/:id", requireAdmin, async (req, res) => {
    try {
      ...
      const salaryUpdate = data.salary !== undefined ? Number(data.salary) : undefined;
  ...
  console.log('[routes] PUT /api/employees/:id updated employee:', employee);
      ...
      // If salary was updated, sync it to the profile table
      if (salaryUpdate !== undefined) {
        try {
          ...
          console.log(`[routes] Synced salary ${salaryUpdate} to profile for employee ${employee._id}`);
        } catch (profileError) {
          console.error('[routes] Failed to sync salary to profile:', profileError);
        }
      }
...
app.get("/api/leave-requests", requireAuth, async (req, res) => {
  try {
    ...
    if (user.role === "admin") {
      // Admin sees all leave requests with employee info
      const requests = await storage.getAllLeaveRequests(); // make sure this method populates employee
      res.json(requests);
    } else {
      ...
      const requests = await storage.getLeaveRequestsByEmployee(employee._id.toString());
      // Optional: include employee info in each request for consistency
...
app.get("/api/profile", requireAuth, async (req, res) => {
  try {
    ...
    if (!profile) {
      return res.json({
        exists: false,
        employeeId: employee._id.toString()  // ⭐ IMPORTANT: RETURN employeeId
      });
    }

    return res.json({
      ...profile.toObject(),
      employeeId: employee._id.toString()   // ⭐ ENSURE frontend gets correct ID
    });
  ...
  try {
    ...
    const existing = await Profile.findOne({ employeeId: employee._id.toString() });
    if (existing) return res.status(400).json({ message: "Profile already exists" });

    // Create new profile with employeeId
    const data = { ...req.body, employeeId: employee._id.toString() };
    const validatedData = insertProfileSchema.parse(data);

    // Create Mongoose model instance so pre-save hook runs
    const profile = new Profile(validatedData);
    await profile.save(); // ✅ generatedId will be automatically assigned here
...
//     // Ensure generatedId exists
//     if (!profile.generatedId) {
//       const lastProfile = await Profile.findOne({}, {}, { sort: { createdAt: -1 } });
//       let nextNumber = 1;
//       if (lastProfile && lastProfile.generatedId) {
//         const lastNum = parseInt(lastProfile.generatedId.replace("E", ""));
//         nextNumber = lastNum + 1;
//       }
//       profile.generatedId = `E${nextNumber}`;
//     }

//     await profile.save(); // ✅ runs pre-save logic if needed
//     res.json(profile);
//   } catch (error) {
...
app.put("/api/profile/:employeeId", requireAuth, async (req, res) => {
  ...
  try {
    ...
    await storage.updateUserPassword(req.user._id, hashed);
