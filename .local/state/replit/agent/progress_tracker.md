[x] 1. Install the required packages
[x] 2. Configure the workflow properly (set to webview with port 5000)
[x] 3. Get MongoDB connection URL from user
[x] 4. Restart the workflow to see if the project is working
[x] 5. Verify the project is working by checking logs and taking screenshot
[x] 6. Migration completed successfully
[x] 7. Fixed profile image upload issue
[x] 8. Fixed phone number validation error in admin employee edit