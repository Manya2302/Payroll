  ...
  const handleRequestOtp = async (e) => {
    ...
    try {
      const res = await fetch("/api/register/request-otp", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username: username.trim(), password, role, email: email.trim() }),
      });
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      const res = await fetch("/api/register/verify-otp", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username: username.trim(), password, role, email: email.trim(), otp: otp.trim() }),
      });
      ...
      <Card className="w-full max-w-md">
        ...
        <CardContent>
          ...
          <div className="mt-6 text-center text-sm">
            ...
            <br />
            Forgot Password?{" "}
            <Button variant="link" className="p-0 h-auto font-semibold" onClick={() => setLocation("/forgot-password")}>
              Reset Password
            </Button>
          </div>
        </CardContent>
