...
//   useEffect(() => {
//     fetch(`http://localhost:5000/api/query/${id}`)
//       .then(res => res.json())
//       .then(setQuery);
//   }, [id]);
...
//     e.preventDefault();
//     await fetch(`http://localhost:5000/api/query/${id}/answer`, {
//       method: "PATCH",
...
export default function QueryAnswer() {
  const [location, navigate] = useLocation(); // ✅ Added navigate
  const id = location.split("/")[3];
  ...
  const handleSubmit = async (e) => {
    ...
    setTimeout(() => {
      navigate("/admin/queries");  // Redirect to query page
    }, 1000);
  };
  ...
  return (
    <Layout>
      <div className="max-w-xl mx-auto mt-8">
        <Card>
          <CardHeader>
            <CardTitle className="text-2xl text-primary dark:text-blue-300">
              Answer Query
            </CardTitle>
          </CardHeader>
          ...
          <CardContent>
            ...
            {submitted && (
              <div className="mt-4 text-green-600 dark:text-green-400 font-semibold">
                Answer submitted and email sent! Redirecting...
              </div>
            )}
          </CardContent>
        </Card>
